...
import { CarProps, make } from "../../../../Types";
import { features } from "../../../../Types";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import { useState } from "react";
import { ImCancelCircle } from "react-icons/im";
import { useAddNewVihicleMutation } from "../../../../features/api/apiSlice";
import { AddVihicleSchema } from "./validatonSchema";
...
const AddVihicleModal = () => {
  const { isOpen, toggleAddVihicleModal }: any = useContext(AddVihicleContext);

  const [createVihicle, { error, data, isLoading }] =
  ...
  const {
    ...
    resolver: yupResolver<Yup.AnyObject>(AddVihicleSchema),
  });
  ...
  const onSubmit = (data: any) => {
    if (!vihicleImages) {
      setNumberOfImagesError("Select atleast one image");
    }
    ...
    createVihicle(formData);
  };

  if (data) {
    toggleAddVihicleModal();
    toast.success("Vihicle created successfully.");
      ...
      >
        <div className="absolute  max-h-[95vh]  overflow-y-scroll flex flex-col bg-white max-w-4xl my-auto md:mx-auto inset-0 mx-2 px-2 md:px-6 pb-8   rounded-md  ">
          <div className="mb-16">
            {" "}
            <></>
            <ModalButton toggleModal={toggleAddVihicleModal} />
          </div>
          {error && (
